    ...
    <div className="w-full">
      ...
      >
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { delay: 0.3 } }}>
          <Typography variant="subtitle1" color="inherit" className="font-medium opacity-75 mt-16 mx-auto max-w-512">
                  ...
                  >
                    <Card className="rounded-16 shadow">
                      <List component="nav" className="p-0 pt-8">
                        <Typography className="font-medium pl-32 py-16 text-16">{category.title}</Typography>

                        {category.featuredArticles.map((article, index) => (
                          <ListItem key={article.id} onClick={() => handleOpenDialog(article)} className="pl-32" button>

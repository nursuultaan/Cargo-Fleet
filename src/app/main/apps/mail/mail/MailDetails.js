          ...
          {!_.isEmpty(labels) && mail.labels.length > 0 && (
            <div className="flex flex-wrap mt-8 -mx-2">
              {mail.labels.map(label => (
                <MailChip className="mt-4 mx-2" title={labels[label].title} color={labels[label].color} key={label} />
              ))}
            </div>
          )}
        </div>
            ...
            <div className="flex flex-wrap -mx-8">
              {mail.attachments.map(attachment => (
                <div className="w-128 px-8 pb-16" key={attachment.fileName}>
                  <img className="w-full rounded-16" src={attachment.preview} alt={attachment.fileName} />
